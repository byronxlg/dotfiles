...
if [ -n "$usage_data" ] && echo "$usage_data" | jq -e . >/dev/null 2>&1; then
    ...
    out+="${sep}${text}5h${reset} ${five_hour_color}${five_hour_pct}%${reset}"
    ...
    out+="${sep}${text}7d${reset} ${seven_day_color}${seven_day_pct}%${reset}"
    ...
    if [ "$extra_enabled" = "true" ]; then
        ...
        out+="${sep}${text}extra${reset} ${extra_color}\$${extra_used}/\$${extra_limit}${reset}"
    fi
fi

# Time at far right
ps1_time=$(date +%H:%M:%S)
out+="${sep}${sky}${ps1_time}${reset}"
